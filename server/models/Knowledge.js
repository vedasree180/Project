...
const getGraph = async (cb) => {
  ...
  try {
    ...
    cb(null, { nodes: formattedNodes, links: formattedLinks });
  } catch (err) {
    // fallback dataset
    const graph = {
      nodes: [
...
const searchNodes = async (term, cb) => {
  const like = `%${term}%`;
...
// n-hop neighbor BFS
const neighborhood = async (startId, hops, cb) => {
